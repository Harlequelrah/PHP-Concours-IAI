...
$select_nationalite = $mybase->prepare("SELECT DISTINCT nationalite FROM candidats ");
$select_nationalite->execute();
$nationalites = $select_nationalite->fetchAll(PDO::FETCH_COLUMN);

$nombreEtudiantsParNationalite = [];

foreach ($nationalites as $nationalite) {
    $nbr = $mybase->prepare("SELECT count(id_candidat) as nbr FROM candidats WHERE nationalite = :nationalite");
    $nbr->execute(['nationalite' => $nationalite]);
    $nbr_result = $nbr->fetch(PDO::FETCH_ASSOC);
    $nombreEtudiantsParNationalite[$nationalite] = $nbr_result['nbr'];
}
